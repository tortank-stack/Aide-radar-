...
const dbPath=process.argv[3]||'aides.json';
const html=fs.readFileSync(htmlPath,'utf8');
const dbPayload=JSON.parse(fs.readFileSync(dbPath,'utf8'));
const TEST_DB=Array.isArray(dbPayload)?dbPayload:(dbPayload.aides||[]);
if(TEST_DB.length<2000) throw new Error('test database too small: '+TEST_DB.length);
global.__AIDERADAR_TEST_DB=TEST_DB;
...
try{
 ...
 vm.runInThisContext('DB=global.__AIDERADAR_TEST_DB;');
 ...
 if(typeof window.AideRadarV4!=="object" || window.AideRadarV4.version!=="4.0.0") throw Error('V4 compatibility layer missing');
 if(typeof window.AideRadarV5!=="object" || window.AideRadarV5.version!=="5.2.0") throw Error('V5.2 base layer missing');
 ...
 console.log('V5_1_TRUST_AND_ROADMAP_OK',rt.join(' / '));
 ...
 if(typeof window.AideRadarV52!=="object" || window.AideRadarV52.version!=="5.2.0") throw Error('V5.2 usage layer missing');
 if(!html.includes('data-v52-vote') || !html.includes('v52ResultFeedback')) throw Error('V5.2 feedback UI missing');
 console.log('V5_2_PUBLIC_LAYER_OK');
 
 const pNoAge=base({activity:'btp',activityDetail:'Plomberie',companyStatus:'standard',companyAge:'lt1',dept:'89',employees:'1-4',project:'creation',projectDetail:'Créer une entreprise de plomberie',budget:25000,ownerAge:0,jobSeeker:false});
 const tNoAge=DB.filter(a=>(a.deps||[]).includes('ALL')||(a.deps||[]).includes('89')).map(a=>({a,r:scoreAid(a,pNoAge)})).filter(x=>x.r!==null).filter(x=>eligibilitySummary(x.a,x.r,pNoAge).state!=='no').sort((x,y)=>confidenceScore(y.a,y.r,pNoAge)-confidenceScore(x.a,x.r,pNoAge));
 const rowsNoAge=dedupeRankedRows(tNoAge,pNoAge).filter(x=>x.r.score>=50).slice(0,12);
 const pathNoAge=buildFinancingPath(rowsNoAge,pNoAge);
 if(pathNoAge.steps.some(x=>/Pacte création/i.test(x.name))) throw new Error('Unknown-age Pacte still recommended');
 if(pathNoAge.primaryGrant!==0) throw new Error('Unknown-age grant counted in prudent base');
 console.log('UNKNOWN_AGE_SAFETY_OK');
