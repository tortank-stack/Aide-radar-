...
def main():
    ...
    raw=decode(download(args.source))
    reader=csv.DictReader(io.StringIO(raw),dialect=dialect(raw))
    ...
    tmp.replace(path)
    print(json.dumps({'ok':True,'matched':matched,'changed':changed,'disabled':disabled,'source_max_modified':newest,'aides':len(aids)},ensure_ascii=False))
